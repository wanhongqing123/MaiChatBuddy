 ...
 *
 * - 针对腾讯云的推流地址，会采用 QUIC 协议进行加速，配合改进后的 BBR2
 * 带宽测算方案，可以最大限度的利用主播的上行带宽，降低直播卡顿率。
 * - 内嵌套的 Qos
 * 流量控制技术具备上行网络自适应能力，可以根据主播端网络的具体情况实时调节音视频数据量。
 * - 内嵌多套美颜磨皮算法（自然&光滑）和多款色彩空间滤镜（支持自定义滤镜），可以根据需要自行选择。
 * - 企业版包含了基于优图 AI 人脸识别技术的大眼、瘦脸、隆鼻以及动效挂架，只需要购买**优图
 * License**就可以零成本集成。
 * - 支持自定义的音视频采集和渲染，让您可以根据项目需要选择自己的音视频数据源。
 */
LITEAV_EXPORT @interface TXLivePush : NSObject
 ...
 */
- (id)initWithConfig:(TXLivePushConfig *)config NS_DESIGNATED_INITIALIZER;
 ...
 *
 * @param rtmpURL
 * 推流地址，参考文档：[获取推流地址](https://cloud.tencent.com/document/product/267/32720 )。
 *
 ...
 *
 * SDK 会暂时停止摄像头采集，并使用 TXLivePushConfig.pauseImg
 * 中指定的图片作为替代图像进行推流，也就是所谓的“垫片”。 这项功能常用于 App
 * 被切到后台运行的场景，尤其是在 iOS 系统中，当 App 切到后台以后，操作系统不会再允许该 App
 * 继续使用摄像头。 此时就可以通过调用 pausePush() 进入垫片状态。
 *
 ...
 *
 * @note 请注意调用顺序：startPush => ( pausePush => resumePush ) =>
 * stopPush()，错误的调用顺序会导致 SDK 表现异常。
 */
- (void)pausePush;
 ...
 *
 * @note adjustResolution 早期被引入是为了让 TXLivePusher
 * 能够满足视频通话这一封闭场景下的一些需求，现已不推荐使用。
 *       如果您有视频通话的需求，可以使用我们专门为视频通话打造的
 * [TRTC](https://cloud.tencent.com/product/trtc) 服务。 由于目前很多 H5
 * 播放器不支持分辨率动态变化，所以开启分辨率自适应以后，会导致 H5 播放端和录制文件的很多兼容问题。
 */
 ...
 *
 * setMirror
 * 所影响的则是观众端看到的视频效果，如果想要保持观众端看到的效果跟主播端保持一致，需要开启镜像；
 ...
 * 如果希望改变观众端看到的视频画面的方向，比如原来是540x960，希望变成960x540，则可以通过设置
 * TXLivePushConfig 中的 homeOrientation 来实现。
 *
 * <pre>
 * // 竖屏推流（HOME 键在下）
 ...
 * @note 当 distance
 * 为1的时候为最远视角（正常镜头），当为5的时候为最近视角（放大镜头），最大值不要超过5，超过5后画面会模糊不清。
 */
 ...
 * 3.8 设置手动对焦区域
 *
 * SDK 默认使用摄像头自动对焦功能，您也可以通过 TXLivePushConfig 中的 touchFocus
 * 选项关闭自动对焦，改用手动对焦。
 * 改用手动对焦之后，需要由主播自己点击摄像头预览画面上的某个区域，来手动指导摄像头对焦。
 *
 * @note 早期 SDK 版本仅仅提供了手动和自动对焦的选择开关，并不支持设置对焦位置，3.0
 * 版本以后，手动对焦的接口才开放出来。
 */
 ...
 *       2. 出于安装包体积的考虑，仅专业版和企业版两个版本的 LiteAVSDK
 * 支持该功能，直播精简版仅定义了接口但并未实现。
 *       3. 录制过程中请勿动态切换分辨率和软硬编，会有很大概率导致生成的视频异常。
 *
 * @param videoPath 视频录制后存储路径。
 * @return 0：成功；-1：videoPath 为空；-2：上次录制尚未结束，请先调用
 * stopRecord；-3：推流尚未开始。
 */
 ...
 *
 * 在自定义视频采集模式下，SDK
 * 不再继续从摄像头采集图像，只保留编码和发送能力，您需要定时地发送自己采集的 SampleBuffer。
 * 要开启自定义视频采集，需要完成如下两个步骤：
 *
 * 1. 开启自定义采集：给 TXLivePushConfig 中的 customModeType 属性增加 CUSTOM_MODE_VIDEO_CAPTURE
 * 选项，代表开启自定义视频采集。
 * 2. 设定视频分辨率：将 TXLivePushConfig 中的 sampleBufferSize 属性设置为您期望的分辨率。
 *   如果期望编码分辨率跟采集分辨率一致，可以不设置 sampleBufferSize 属性，而是将
 * autoSampleBufferSize 设置为 YES。
 *
 * @note 1. 开启自定义视频采集后，即无需再调用 startPreview 来开启摄像头采集。
 *       2. SDK 内部有简单的帧率控制，如果发送太快时 SDK 会自动丢弃多余的帧；如果超时不发送，SDK
 * 会不断地重复发送的最后一帧。
 *
 ...
 *
 * 在自定义音频采集模式下，SDK
 * 不再继续从麦克风采集声音，只保留编码和发送能力，您需要定时地发送自己采集的声音数据（PCM 格式）
 * 要开启自定义音频采集，需要完成如下两个步骤：
 *
 * 1. 开启自定义采集：给 TXLivePushConfig 中的 customModeType 属性增加 CUSTOM_MODE_AUDIO_CAPTURE
 * 选项，代表开启自定义音频采集。
 * 2. 设定音频采样率：将 TXLivePushConfig 中的 audioSampleRate
 * 属性设置为您期望的音频采样率，audioChannels 设置为期望的声道数，默认值：1（单声道）。
 *
 ...
 * 7.3 自定义视频处理回调
 *
 * 自定义视频采集和自定义视频处理不能同时开启，与自定义视频采集不同，自定义视频处理依然是由 SDK
 * 采集摄像头的画面， 但 SDK 会通过
 * TXVideoCustomProcessDelegate（见“TXVideoCustomProcessDelegate.h”）回调将数据回调给您的 App
 * 进行二次加工。
 *
 * 如果要开启自定义视频处理，需要给 TXLivePushConfig 中的 customModeType 属性增加
 * CUSTOM_MODE_VIDEO_PREPROCESS 选项。
 *
 ...
 * 7.4 自定义音频处理回调
 *
 * 自定义音频采集和自定义音频处理不能同时开启，与自定义音频采集不同，自定义音频处理依然是由 SDK
 * 采集麦克风的声音， 但 SDK 会通过
 * TXAudioCustomProcessDelegate（见“TXAudioCustomProcessDelegate.h”）回调将数据回调给您的 App
 * 进行二次加工。
 *
 ...
 *       - sendMessageEx 发送消息给旧版本的5.0 及以前的 SDK
 * 版本时，消息会无法正确解析，但播放不受影响。
 */
...
/**
 * 11.1 自定义音频采集，向 SDK 发送自己采集的音频数据
 *
 * @deprecated 从 9.9 版本开始，本接口已不再支持。
 */
#if TARGET_OS_IPHONE
