 ...
 *
 * // Select from a string returned by liteav_av_hash_names()
 * hash_name = ...;
 *
 * // Allocate a hash context
 * ret = liteav_av_hash_alloc(&ctx, hash_name);
 * if (ret < 0)
 *     return ret;
 *
 * // Initialize the hash context
 * liteav_av_hash_init(ctx);
 *
 * // Update the hash context with data
 * while (data_left) {
 *     liteav_av_hash_update(ctx, data, size);
 * }
 *
 ...
 * // liteav_av_malloc().
 * output_buf = liteav_av_malloc(liteav_av_hash_get_size(ctx));
 * if (!output_buf)
 *     return AVERROR(ENOMEM);
 *
 * // Finalize the hash context.
 * // You can use any of the liteav_av_hash_final*() functions provided, for other
 ...
 * // memory needed.
 * liteav_av_hash_final(ctx, output_buffer);
 *
 * // Free the context
 * liteav_av_hash_freep(&ctx);
 * @endcode
 *
 ...
 * call liteav_av_hash_init() to do so.
 */
int liteav_av_hash_alloc(struct AVHashContext **ctx, const char *name);
 ...
 */
const char *liteav_av_hash_names(int i);

/**
 * Get the name of the algorithm corresponding to the given hash context.
 */
const char *liteav_av_hash_get_name(const struct AVHashContext *ctx);

/**
 * Maximum value that liteav_av_hash_get_size() will currently return.
 *
 ...
 * liteav_av_hash_get_size(), or limit your use of the Hashing API to hashes that are
 ...
 */
int liteav_av_hash_get_size(const struct AVHashContext *ctx);
 ...
 */
void liteav_av_hash_init(struct AVHashContext *ctx);
...
void liteav_av_hash_update(struct AVHashContext *ctx, const uint8_t *src, int len);
#else
void liteav_av_hash_update(struct AVHashContext *ctx, const uint8_t *src, size_t len);
#endif

/**
 * Finalize a hash context and compute the actual hash value.
 *
 * The minimum size of `dst` buffer is given by liteav_av_hash_get_size() or
 ...
 *
 * @see liteav_av_hash_final_bin() provides an alternative API
 */
void liteav_av_hash_final(struct AVHashContext *ctx, uint8_t *dst);
 ...
 * been finalized.
 *
 * If `size` is smaller than the hash size (given by liteav_av_hash_get_size()), the
 ...
 */
void liteav_av_hash_final_bin(struct AVHashContext *ctx, uint8_t *dst, int size);
 ...
 * value returned by liteav_av_hash_get_size(), the string will be truncated.
 *
 * @param[in,out] ctx  Hash context
 * @param[out]    dst  Where the string will be stored
 * @param[in]     size Maximum number of bytes to write to `dst`
 */
void liteav_av_hash_final_hex(struct AVHashContext *ctx, uint8_t *dst, int size);
 ...
 * the value returned by liteav_av_hash_get_size(), the string will be truncated.
 *
 * @param[in,out] ctx  Hash context
 * @param[out]    dst  Where the final hash value will be stored
 * @param[in]     size Maximum number of bytes to write to `dst`
 */
void liteav_av_hash_final_b64(struct AVHashContext *ctx, uint8_t *dst, int size);
 ...
 */
void liteav_av_hash_freep(struct AVHashContext **ctx);
